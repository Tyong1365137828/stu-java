...
//		request.setCharacterEncoding("utf-8");
		...
		if(user != null){//如果不是空，即合法用户
			...
			//3、设定卡的使用范围，即访问路径
			setcook_id.setPath("/EE_9_FilterAndListen/");
			setcook_pwd.setPath("/EE_9_FilterAndListen/");
			...
			System.out.println("LoginServlet的user"+user);
//			//或者使用
//			request.getSession().setAttribute("sion_user", user);
			...
			Integer count = (Integer) servletContext.getAttribute("login_count");//不能用int
			if(count == null){//如果是访问用户为空,即是第一个
				count=1;
			}else{//否则
				count++;
			}
			servletContext.setAttribute("login_count", count);

		...
		if (e.getSource().equals(confirmButton)) {
			...
			if (passwordField.getText().length() == 0) {
				...
				try {
					...
					String sql = "select * from stu_id_key where stu_id = '" + id + "'" + "and stu_password = '"
							+ password + "'";
					...
					if(count != 0) {
						...
						JOptionPane.showMessageDialog(p2, "'"+id+"'同学将初始的密码'"+password+"'更改为'" + repassword + "'", "信息提示",
								JOptionPane.WARNING_MESSAGE);
					}else if(count == 0){
						JOptionPane.showMessageDialog(p2, "当前密码不正确,无法确定是否为本人,所以无法进行密码重设", "错误提示", JOptionPane.ERROR_MESSAGE);
					}

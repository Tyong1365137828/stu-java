...
public class AdminChangePass implements ActionListener{
	
	JFrame changepassFrame = new JFrame("管理员修改密码界面");
	...
	public AdminChangePass() {
		...
		changepassFrame.setSize(500, 600);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == confirmButton) {
			...
			if (passwordField.getText().length() == 0) {
				...
				JOptionPane.showMessageDialog(p2, "两次密码不一样！", "错误提示", JOptionPane.ERROR_MESSAGE);
			} else {
				try {
					GetConn getConn = new GetConn();
					Connection connection = getConn.getConnection();

					String sql = "select * from administrator where adm_account='" + id + "'" + "and adm_password ='" + password
							+ "'";
					...
					if(count != 0) {
						...
						administratorDaoImpl.UpdateAdm(id, repassword);
						
						JOptionPane.showMessageDialog(p2, "'"+id+"'管理员，您初始的密码'"+password+"'更改为'" + repassword + "'", "信息提示",
								JOptionPane.WARNING_MESSAGE);
					}else if(count == 0){
						JOptionPane.showMessageDialog(p2, "当前密码不正确,无法确定是否为本人,所以无法进行密码重设，如果你忘记密码，请前往登录界面点忘记密码进行找回！", "错误提示", JOptionPane.ERROR_MESSAGE);
					}

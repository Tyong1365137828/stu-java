...
public class AdminAddStuAward implements ActionListener {

	JFrame addAwardStuFrame = new JFrame("学生奖励界面");
	...
	public AdminAddStuAward() {
			...
			public void itemStateChanged(ItemEvent e) {
				...
				if (e.getStateChange() == ItemEvent.SELECTED) {
					...
					if (e.getItem().equals(award1list[1])) {
						awardField1.setText("三好学生");
					}
					if (e.getItem().equals(award1list[2])) {
						awardField1.setText("优秀班干部");
					}
					if (e.getItem().equals(award1list[3])) {
						awardField1.setText("优秀干事");
					}
				}
		 ...
		 * 
		 */
		addAwardStuFrame.add(showPanel);
		addAwardStuFrame.setSize(500, 400);
		addAwardStuFrame.setVisible(true);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == addButton1) {
			...
			if(awardField1.getText().equals("请选择")){
				...
				AdministratorDaoImpl administratorDaoImpl = new AdministratorDaoImpl();
				administratorDaoImpl.AddAwardForStu(id, award);

				JOptionPane.showMessageDialog(p13, "学生奖励成功", "信息提示框", JOptionPane.WARNING_MESSAGE);
			}
		...
		if (e.getSource() == checkButton2) {
			if (field2.getText().length() == 0) {
				...
				try {
					...
					if (count == 0) {
						JOptionPane.showMessageDialog(p22, "没有该学生", "信息提示框", JOptionPane.WARNING_MESSAGE);
					} else {
						while (resultSet.next()) {
							String name = resultSet.getString(2);

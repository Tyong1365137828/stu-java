...
public class AdminAddStu implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == addButton) {
			if(idField.getText().length() == 0) {
				...
				if(count != 0) {
					JOptionPane.showMessageDialog(p2, "已存在id为"+id+"的学生账号,请更换别的账号", "消息提示", JOptionPane.WARNING_MESSAGE);
				}else {
					System.out.println("depnum="+depnum);
					
					AdministratorDaoImpl administratorDaoImpl2 = new AdministratorDaoImpl();
					administratorDaoImpl2.AddStu(id, password, depnum);
